...
class ProgressScreen extends StatelessWidget {
  final Lang l1, l2;
  final Job job;
  const ProgressScreen({super.key, required this.l1, required this.l2, required this.job});

  @override
  Widget build(BuildContext context) {
    final covered = situations.where((s) => s.readiness >= 1).length;
    final overall =
        situations.map((s) => s.readiness).fold(0.0, (a, b) => a + b) / situations.length;
    ...
    return SafeArea(
      ...
      child: ListView(
        ...
        children: [
          ...
          Text('${l1.english} to ${l2.english} · ${job.title}',
          ...
          Container(
            ...
            child: Column(children: [
              ...
              Text('${(overall * 100).round()}% ready for work',
                  style: Boli.head(25, weight: 700)),
              const SizedBox(height: 4),
              Text('$covered of ${situations.length} situations you can handle unaided',
                  textAlign: TextAlign.center, style: Boli.body(15, color: Boli.inkSoft)),
              const SizedBox(height: 16),
              HandloomBorder(color: Boli.marigold.withValues(alpha: .55), height: 10),
              const SizedBox(height: 16),
              Row(children: [
          ...
          Container(
            ...
            child: Column(children: [
              _Sound(a: 'ट', b: 'त', label: 'Retroflex vs dental', score: .42),
              const SizedBox(height: 14),
              ...
              Container(
                ...
                child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Icon(Icons.lightbulb_rounded, size: 17, color: Boli.marigold),
                  ...
                  Expanded(
                    child: Text(
                        ...
                        style: Boli.body(13.5, color: Boli.inkSoft, height: 1.45)),
                  ),
                ]),
              ),
            ]),
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                const Icon(Icons.verified_rounded, color: Boli.marigold, size: 26),
                const SizedBox(width: 10),
                Expanded(
                  child: Text('Skill certificate', style: Boli.head(21, weight: 700, color: Boli.cream)),
                ),
                Container(
                  ...
                  child: Text('LEVEL 2', style: Boli.label(color: Boli.marigold, size: 11)),
                ),
              ]),
              const SizedBox(height: 12),
              Text('Functional ${l2.english} · ${job.title} register',
                  style: Boli.body(16, weight: FontWeight.w700, color: Boli.cream)),
              ...
              Text(
                  ...
                  style: Boli.body(14, color: Boli.cream.withValues(alpha: .72), height: 1.45)),
              const SizedBox(height: 16),
              Row(children: [
                Expanded(
                  child: Container(
                    ...
                    child: FractionallySizedBox(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                            color: Boli.marigold, borderRadius: BorderRadius.circular(5)),
                      ),
                    ),
                  ),
                ),
                const SizedBox(width: 12),
                Text('$covered / 8', style: Boli.body(14, weight: FontWeight.w800, color: Boli.cream)),
              ]),
            ]),
          ),
          const SizedBox(height: 20),
          ...
          Container(
            decoration: Boli.card(),
            child: Column(children: [
              _Toggle(icon: Icons.record_voice_over_rounded, title: 'Voice only', sub: 'No reading needed anywhere', on: false),
              Divider(height: 1, color: Boli.sand, thickness: 1.5),
              _Toggle(icon: Icons.text_fields_rounded, title: 'Bigger text', sub: 'Easier in bright sun', on: true),
              Divider(height: 1, color: Boli.sand, thickness: 1.5),
              _Toggle(icon: Icons.slow_motion_video_rounded, title: 'Slower speech', sub: 'Play audio at 0.7×', on: true),
            ]),
          ),
        ],
      ),
...
class _Stat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(
        child: Column(children: [
          ...
          Text(label,
              textAlign: TextAlign.center, style: Boli.label(size: 10, color: Boli.inkSoft)),
        ]),
      );
}
...
class _Sound extends StatelessWidget {
  ...
  final double score;
  const _Sound({required this.a, required this.b, required this.label, required this.score});
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          LayoutBuilder(
            builder: (_, box) => Container(
              height: 7,
              decoration:
                  BoxDecoration(color: Boli.sand, borderRadius: BorderRadius.circular(5)),
              child: Align(
                alignment: Alignment.centerLeft,
                child: Container(
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: Padding(
          ...
          child: Row(children: [
            ...
            Expanded(child: Text(lang.english, style: Boli.body(15, color: Boli.inkSoft))),
            if (lang.installed)
              Row(children: [
                const Icon(Icons.offline_pin_rounded, size: 19, color: Boli.leaf),
                const SizedBox(width: 6),
                Text('On phone',
                    style: Boli.body(13.5, weight: FontWeight.w700, color: Boli.leaf)),
              ])
            else
              Row(children: [
                Text('${lang.mb} MB', style: Boli.body(13.5, color: Boli.inkSoft)),
                const SizedBox(width: 8),
                const Icon(Icons.download_rounded, size: 20, color: Boli.inkSoft),
              ]),
          ]),
        ),
      );
...
class _Toggle extends StatefulWidget {
  ...
  final bool on;
  const _Toggle({required this.icon, required this.title, required this.sub, required this.on});
  @override
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(widget.title, style: Boli.body(16, weight: FontWeight.w700)),
