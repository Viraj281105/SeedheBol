...
class Shell extends StatefulWidget {
  final Lang l1, l2;
  final Job job;
  const Shell({super.key, required this.l1, required this.l2, required this.job});
...
class TodayScreen extends StatefulWidget {
  final Lang l1, l2;
  final Job job;
  const TodayScreen({super.key, required this.l1, required this.l2, required this.job});
        ...
        transitionsBuilder: (_, a, __, c) {
          ...
          return FadeTransition(
            ...
            child: SlideTransition(
                position: Tween(begin: const Offset(0, .06), end: Offset.zero).animate(cu), child: c),
          );
        },
      ),
    );
          ...
          children: [
            Center(
              child: Container(
                  width: 42, height: 5,
                  decoration: BoxDecoration(
                      color: Boli.sand, borderRadius: BorderRadius.circular(4))),
            ),
            ...
            const SizedBox(height: 16),
            Text('${s.phrases} phrases are written for this situation. '
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Row(children: [
                            Text(widget.l1.native, style: Boli.body(15, weight: FontWeight.w700)),
                            const Padding(
                              padding: EdgeInsets.symmetric(horizontal: 7),
                              child: Icon(Icons.arrow_forward_rounded, size: 15, color: Boli.terracotta),
                            ),
                            Text(widget.l2.native,
                                style: Boli.body(15, weight: FontWeight.w700, color: Boli.terracotta)),
                          ]),
                          const SizedBox(height: 2),
                          Text(widget.job.title, style: Boli.body(13.5, color: Boli.inkSoft)),
                        ],
                      ),
                    ),
                    Container(
                      ...
                      child: Row(children: [
                        const Icon(Icons.cloud_off_rounded, size: 16, color: Boli.leaf),
                        const SizedBox(width: 6),
                        Text('Offline',
                            style: Boli.body(13.5, weight: FontWeight.w800, color: Boli.leaf)),
                      ]),
                    ),
                  ],
                ),
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text('DAY $_day OF $_ofDays',
                                    style: Boli.label(color: Boli.marigold, size: 11.5)),
                                const SizedBox(height: 5),
                                Text('${(_overall * 100).round()}% ready for work',
                                    style: Boli.head(25, weight: 700, color: Boli.cream)),
                                const SizedBox(height: 3),
                                Text(
                                    '${situations.where((s) => _read(s) >= 1).length} of ${situations.length} situations covered',
                                    style: Boli.body(13.5,
